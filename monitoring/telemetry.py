...
class TelemetryCollector:
    ...
    def __init__(self):
        ...
        self.traditional_log_file = None
        # Don't set up logging automatically - wait for enable() call
        
    def enable_logging(self):
        """Enable telemetry logging (call this in debug mode)"""
    ...
    def _setup_traditional_logging(self):
        ...
        self.traditional_log_file = f"data/telemetry/telemetry_dump_{timestamp}.log"
        
        # Ensure directory exists
        import os
        os.makedirs("data/telemetry", exist_ok=True)
        
        # Create custom formatter for handling long prompts
        class CompactPromptFormatter(logging.Formatter):
            def format(self, record):
                # Format the record normally first
                formatted = super().format(record)
                
                # Handle long prompts/templates by compacting them
                if ("Extracting blocks from template:" in formatted or 
                    "Rendered prompt:" in formatted):
                    # Find the message part after the colon
                    ...
                    if len(parts) > 1:
                        ...
                        formatted = prefix + compact_content
                
                return formatted
        
        # Configure comprehensive logging for maximum debugging
        handler = logging.FileHandler(self.traditional_log_file)
        handler.setFormatter(CompactPromptFormatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
    ...
    def process_kernel_response(self, response, user_input: str, context: Dict[str, Any] = None):
        """Process response from kernel.invoke() and extract telemetry data"""
        try:
            # Track the response event
            response_event = self._create_event("KERNEL_RESPONSE", {
                "user_input": user_input,
                "context": context or {}
            })
            ...
            if hasattr(response, 'value'):
                # Handle list of messages or single message
                ...
                for message in messages:
                    ...
                    if hasattr(message, 'content'):
                        ...
                        # Track AI response
                        self.ai_response(content, context.get('model', 'gpt-4o-mini'))
                        
                        # Extract token usage if available
                        if hasattr(message, 'metadata') and 'usage' in message.metadata:
                    ...
                    # Extract function calls from message items
                    if hasattr(message, 'items'):
                        for item in message.items:
                            if hasattr(item, 'function_call_result'):
                                ...
                                self._create_event("FUNCTION_EXECUTED", {
            ...
            self.error("process_kernel_response", str(e), {"response": str(response)})
            return None
    
    def local_function_log(self, source: str, message: str, data: Dict[str, Any] = None):
        """Log events from local code execution (e.g., DataManager functions)"""
        return self._create_event("LOCAL_FUNCTION", {
            "source": source,
            "message": message,
            "data": data or {},
            "timestamp": datetime.now().isoformat()
    ...
    def to_timestamped_log(self, base_filename: str = "telemetry"):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{base_filename}_structured_{timestamp}.log"
    ...
    def get_traditional_log_filename(self) -> str:
        ...
        return self.traditional_log_file or "No traditional log (logging not enabled)"
